...
function ProductsList({products, search}) {
  ...
  return (
    <div className={productsListStyles.productsList}>
     {products.map(product=><ProductCard key={product.id} name={product.name} brand={product.brand} price={product.price} image={product.thumbnail} search={search} id={product.id}></ProductCard> )}
    </div>
  );
}
